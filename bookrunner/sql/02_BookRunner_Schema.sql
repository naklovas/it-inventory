...
BEGIN
    ...
    -- Dinamik SQL: GroupName kolonu yeni kurulumlarda hic var olmadigindan,
    -- statik bir UPDATE derleme (parse) zamaninda "Invalid column name" hatasi
    -- verir - IF bloguna girilmese bile. EXEC() ile derleme calisma zamanina
    -- ertelenir, boylece kolon gercekten varsa calisir, yoksa hic derlenmez.
    EXEC(N'UPDATE [bookrunner].[RoleMappings] SET [TeamName] = [GroupName] WHERE [TeamName] IS NULL;');
...
-- Mevcut kurulumlarda Tasks tablosu gorev bazinda "basarisiz olursa" otomatik
-- eylem alanlari olmadan olusturulmus olabilir (bkz. RunbookTask.FailureAction).
IF COL_LENGTH(N'bookrunner.Tasks', 'FailureAction') IS NULL
BEGIN
    ALTER TABLE [bookrunner].[Tasks] ADD [FailureAction] int NOT NULL CONSTRAINT [DF_Tasks_FailureAction] DEFAULT (0);
END
GO
...
BEGIN
    -- Dinamik SQL: DependsOnTaskId kolonu yeni kurulumlarda Tasks tablosunda hic
    -- var olmadigindan, statik bir INSERT/SELECT derleme (parse) zamaninda
    -- "Invalid column name" hatasi verir - IF bloguna girilmese bile. EXEC() ile
    -- derleme calisma zamanina ertelenir, boylece kolon gercekten varsa calisir,
    -- yoksa hic derlenmez.
    EXEC(N'
...
BEGIN
    ...
    IF NOT EXISTS (
        SELECT 1 FROM [bookrunner].[__EFMigrationsHistory]
        WHERE [MigrationId] = N'20260909084838_AddTaskFailureAction'
    )
    BEGIN
        INSERT INTO [bookrunner].[__EFMigrationsHistory] ([MigrationId], [ProductVersion])
