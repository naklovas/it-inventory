...
public sealed class BookRunnerApiClient(HttpClient httpClient, ILogger<BookRunnerApiClient> logger)
{
    ...
    public Task DeactivateRollbackAsync(Guid runbookId, bool deleteSteps, CancellationToken ct = default)
        => PostAsync<object?, object>($"api/runbooks/{runbookId}/deactivate-rollback?deleteSteps={deleteSteps}", null, ct);
    ...
    public Task<IReadOnlyList<ScenarioDto>?> ListScenariosAsync(Guid runbookId, CancellationToken ct = default)
        => GetAsync<IReadOnlyList<ScenarioDto>>($"api/runbooks/{runbookId}/scenarios", ct);
